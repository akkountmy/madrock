/**
 * Титульный блок: содержимое ложится на фотографию (версия с учётом
 * фактического CSS после смены палитры).
 *
 * Было: затемнение шло сверху вниз и на середине было уже почти чёрным —
 * фотография читалась как тёмное поле. Стало: затемнение слева направо
 * (текст в тёмной части, снимок виден справа), лёгкая виньетка и высота,
 * чтобы фотография занимала заметную площадь.
 *
 * Запуск: node tools/fix-hero-2.mjs
 */
...
.hero::after {
  ...
  background:
    radial-gradient(120% 90% at 15% 10%, rgba(var(--accent-rgb), .16), transparent 55%),
...
const problems = []
let applied = 0
const EDITS = [['src/styles.css', CSS_FROM, CSS_TO, 1]]
