/**
 * Серьёзный стиль служебных кнопок: «Копировать» в реферальном блоке,
 * «Применить» у купонов, «Выйти» и переключатель входа.
 *
 * Было: размытые пастельные кнопки с круглыми краями. Стало: плотные формы
 * в бренде логотипа — чёрный, красный, белый, капслок с разрядкой, радиус 10 px.
 * У «Копировать» появилось состояние «Скопировано».
 *
 * Запуск: node tools/fix-buttons.mjs
 */
...
const EDITS = [
  // 1. реферальный блок: поле в рамке + плотная брендовая кнопка
  ['src/app.js',
    "'<div class=\"ref-box\"><input class=\"input\" id=\"ref-input\" readonly value=\"' + esc('madrock.by/r/' + user.ref) + '\">' +\n            '<button class=\"btn btn--gold\" id=\"ref-copy\" type=\"button\">Копировать</button></div>'",
    "'<div class=\"ref-box\"><input class=\"input ref-box__field\" id=\"ref-input\" readonly value=\"' + esc('madrock.by/r/' + user.ref) + '\" aria-label=\"Ваша реферальная ссылка\">' +\n            '<button class=\"btn btn--brand\" id=\"ref-copy\" type=\"button\"><span>Копировать</span></button></div>'", 1],

  // 2. поведение копирования
  ['src/app.js', OLD_COPY, NEW_COPY, 1],

  // 3. купоны и выход — тот же служебный стиль
  ['src/app.js',
    "'<button class=\"btn btn--sm btn--soft\" data-coupon=\"' + esc(c.code) + '\" ' + (c.used ? 'disabled' : '') + ' type=\"button\">' + (c.used ? 'использован' : 'применить') + '</button>'",
    "'<button class=\"btn btn--outline\" data-coupon=\"' + esc(c.code) + '\" ' + (c.used ? 'disabled' : '') + ' type=\"button\">' + (c.used ? 'Использован' : 'Применить') + '</button>'", 1],
  ['src/app.js',
    "'<button class=\"btn btn--sm btn--soft\" id=\"acc-logout\" type=\"button\">Выйти</button>'",
    "'<button class=\"btn btn--outline\" id=\"acc-logout\" type=\"button\">Выйти</button>'", 1],

  // 4. в модальном окне напитка «Позже» читалось двусмысленно
  ['src/app.js', '<button class="btn btn--ghost" id="item-one" type="button">Позже</button>',
    '<button class="btn btn--ghost" id="item-one" type="button">Отмена</button>', 1],
]

const CSS = `
/* --- служебные кнопки в бренде логотипа -----------------------------------
   Плотная форма, капслок с разрядкой, чёткий край: так кнопки читаются как
   рабочая часть интерфейса, а не как декоративный элемент. */
.btn--brand {
...
for (const [file, from, to, expected] of EDITS) {
  ...
  if (found !== expected) { problems.push(`${file}: «${from.slice(0, 66).replace(/\n/g, '⏎')}…» — найдено ${found}`); continue }
